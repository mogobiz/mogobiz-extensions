...
class BrandRiver extends AbstractESRiver<Brand> {
    ...
    Observable<Brand> retrieveCatalogItems(final RiverConfig config){
        ...
        if(!_languages.flatten().isEmpty()) {
            Translation.executeQuery('select t from Brand brand, Translation t where t.target=brand.id and t.lang in :languages and brand.company in (select c.company from Catalog c where c.id in (:idCatalogs))',
        ...
        Product.executeQuery('select p, brand.uuid, category from Product p left join fetch p.category as category left join fetch p.brand as brand left join fetch category.parent WHERE category.catalog.id in (:idCatalogs) and p.state = :productState and p.deleted = false',
        ...
        return Observable.from(Brand.executeQuery('select brand from Brand brand left join fetch brand.brandProperties where brand.company in (select c.company from Catalog c where c.id in (:idCatalogs))',
