...
class SkuRiver  extends AbstractESRiver<TicketType>{
    ...
    Observable<TicketType> retrieveCatalogItems(RiverConfig config) {
        ...
        if(!_languages.flatten().isEmpty()){
            ...
            Translation.executeQuery('select t from TicketType sku, Translation t where (t.target=sku.variation1.id or t.target=sku.variation1.variation.id or t.target=sku.variation2.id or t.target=sku.variation2.variation.id or t.target=sku.variation3.id or t.target=sku.variation3.variation.id) and t.lang in :languages and (sku.product.category.catalog.id=:idCatalog and sku.product.state=:productState) and (sku.stopDate is null or sku.stopDate >= :today)',
                    [languages:_languages, idCatalog:config.idCatalog, productState:ProductState.ACTIVE, today: now], [readOnly: true, flushMode: FlushMode.MANUAL]).groupBy {it.target.toString()}.each {k, v -> TranslationsRiverCache.instance.put(k, v)}
        ...
        Observable.from(TicketType.executeQuery(
                'SELECT sku FROM TicketType sku ' +
                        ...
                        'WHERE p.category.catalog.id=:idCatalog and p.state = :productState and p.deleted = false and (sku.stopDate is null or sku.stopDate >= :today)',
