...
class SuggestionRiver extends AbstractESRiver<Suggestion>{
    ...
    Observable<Suggestion> retrieveCatalogItems(RiverConfig config) {
        ...
        return Observable.from(
                config.partial ? Suggestion.executeQuery('FROM Suggestion s ' +
                ...
                'WHERE s.pack.category.id in (:idCategories) and p.state = :productState',
                [idCategories:config.idCategories, productState:ProductState.ACTIVE], args) :
                Suggestion.executeQuery('FROM Suggestion s ' +
                'join fetch s.pack as pack ' +
                'join fetch s.product as p ' +
                'left join fetch p.ticketTypes ' +
                'left join fetch p.features ' +
